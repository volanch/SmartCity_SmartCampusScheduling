...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        var datasets = root.getAsJsonArray("datasets");
        System.out.println("Total datasets: " + datasets.size());

        for (int dsIndex = 0; dsIndex < datasets.size(); dsIndex++) {
            System.out.println("\n==============================");
            System.out.println("Processing dataset #" + (dsIndex + 1));
            System.out.println("==============================");

            JsonObject ds = datasets.get(dsIndex).getAsJsonObject();
            UnifiedLoader.Loaded loaded = UnifiedLoader.fromJson(ds);
            ...
            int sourceVertex = ds.has("source") ? ds.get("source").getAsInt() : 0;
            if (sourceVertex < 0 || sourceVertex >= g.size()) {
                System.out.println("[WARN] Invalid source in dataset, defaulting to 0.");
                sourceVertex = 0;
            }
